...
enum SpeakerAttributor {
    ...
    /// imports) — the caller just keeps `.channels.json` attribution.
    static func attribute(meetingId: String, audioURL: URL) async {
        guard Prefs.diarizationEnabled else { return }
        guard let models = await DiarizerModelManager.shared.readyModels else { return }
        guard let tracks = try? AudioFileDecoder.decodeStereoTracks(url: audioURL), !tracks.sys.isEmpty else { return }
        ...
        guard let result = try? manager.performCompleteDiarization(tracks.sys, sampleRate: 16_000),
              !result.segments.isEmpty else { return }
