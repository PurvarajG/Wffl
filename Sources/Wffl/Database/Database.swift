...
final class Database: @unchecked Sendable {
    ...
    /// Appends ledger rows for a stage that runs *after* the segment set is
    /// already on disk (the cleanup pipeline), so it can't ride along with
    /// `replaceSegments`. One transaction, all-or-nothing: a half-written
    /// ledger is worse than none, because the pack seeder downstream would
    /// read it as complete evidence.
    ///
    /// P0: since T-05 removed the per-segment corrector, the cleanup pipeline
    /// is the *only* remaining producer of provenance evidence. Before this
    /// existed there were no call sites constructing a `TranscriptEdit` at
    /// all — the schema, the model and the read path were live but the ledger
    /// was never written, so "seed the pack from observed corrections" had no
    /// observations to work from.
    func appendEdits(_ edits: [TranscriptEdit]) {
    ...
    /// Every accepted correction ever recorded, newest first, for the
    /// pack-seeding review queue (P0/P2). Rejected proposals are deliberately
    /// excluded — a rejection is evidence the guard worked, not evidence of a
    /// real mishearing.
    func acceptedEdits(limit: Int = 500) -> [TranscriptEdit] {
        // `limit` is an internal Int, never user text, and is clamped here
        // rather than bound — `SQLValue` has no integer case and binding a
        // Double to LIMIT is a needless dependency on SQLite's coercion.
        let bounded = max(1, min(limit, 10_000))
        return query(
            "SELECT id,meeting_id,segment_id,stage,old,new,model,confidence,accepted,reject_reason,created_at FROM transcript_edits WHERE accepted = 1 ORDER BY created_at DESC LIMIT \(bounded)"
        ) { s in
            TranscriptEdit(
    ...
    /// Deletes auto-named `Speaker N` rows that no segment references any
    /// more, and returns how many went (P4).
    ///
    /// Auto-named speakers are meeting-local placeholders that
    /// `VoiceLibrary.matchCandidates` deliberately excludes from cross-meeting
    /// matching — so once nothing points at one it is unreachable by every
    /// code path and exists only to push the next auto-name higher. That is
    /// how a six-minute meeting with a single voice came to be labelled
    /// "Speaker 16": the suffix counted every placeholder ever created, not
    /// clusters in the recording. Re-running diarization on a meeting orphans
    /// its previous placeholders, so without this the count only ever climbs.
    ///
    /// Named speakers are never touched, even when unreferenced — a person the
    /// user named is a library entry that should survive deleting the meeting
    /// they were first heard in. "Me" is likewise reserved.
    @discardableResult
    func pruneUnreferencedAutoSpeakers() -> Int {
        let doomed = allSpeakers()
            .filter { $0.id != Speaker.meId && VoiceLibrary.isAutoGeneratedName($0.name) }
            .map(\.id)
        guard !doomed.isEmpty else { return 0 }
        let referenced = Set(query(
            "SELECT DISTINCT speaker_id FROM transcript_segments WHERE speaker_id IS NOT NULL"
        ) { self.col($0, 0) })
        var removed = 0
        for id in doomed where !referenced.contains(id) {
            deleteSpeaker(id: id)
            removed += 1
        }
        return removed
    ...
    /// Removes a speaker row outright. Segments pointing at it are left with a
    /// dangling `speaker_id`, so only call this for a speaker nothing
    /// references — `pruneUnreferencedAutoSpeakers` checks that first.
    func deleteSpeaker(id: String) {
        guard id != Speaker.meId else { return }
        run("DELETE FROM speakers WHERE id = ?", [.text(id)])
    }
