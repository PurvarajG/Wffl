...
/// Thin thread-safe SQLite wrapper + repositories for meetings, transcripts, summaries.
/// Mirrors Meetily's local-first storage: everything lives in Application Support.
final class Database: @unchecked Sendable {
    static let shared = Database()

    private var db: OpaquePointer?
    private let queue = DispatchQueue(label: "meetily.db")
    private let SQLITE_TRANSIENT = unsafeBitCast(-1, to: sqlite3_destructor_type.self)

    static var appSupportDir: URL {
        let base = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
        let dir = base.appendingPathComponent("MeetilyMac", isDirectory: true)
    ...
    private init() {
        let path = Database.appSupportDir.appendingPathComponent("meetily.sqlite").path
        if sqlite3_open(path, &db) != SQLITE_OK {
            NSLog("Meetily: failed to open database at \(path)")
        }
    ...
    private func exec(_ sql: String) -> Bool {
        ...
        if !ok, let err { NSLog("Meetily SQL error: \(String(cString: err))"); sqlite3_free(err) }
        return ok
    }
    ...
    private func run(_ sql: String, _ params: [SQLValue]) {
        queue.sync {
            ...
            guard sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK else {
                NSLog("Meetily prepare failed: \(String(cString: sqlite3_errmsg(db)))"); return
            }
            defer { sqlite3_finalize(stmt) }
            bind(stmt, params)
            if sqlite3_step(stmt) != SQLITE_DONE {
                NSLog("Meetily step failed: \(String(cString: sqlite3_errmsg(db)))")
            }
        }
    }
    ...
    private func query<T>(_ sql: String, _ params: [SQLValue] = [], map: (OpaquePointer) -> T) -> [T] {
        queue.sync {
            ...
            var stmt: OpaquePointer?
            guard sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK else {
                NSLog("Meetily prepare failed: \(String(cString: sqlite3_errmsg(db)))"); return out
            }
            defer { sqlite3_finalize(stmt) }
            bind(stmt, params)
